...
// ОТПРАВКА УВЕДОМЛЕНИЯ В TELEGRAM (исправленная функция)
async function sendTelegramNotification(recipientUserId, chat, message, senderUserId) {
    try {
        ...
        // 1. Проверяем, является ли recipientUserId Telegram ID (числом)
        if (!isNaN(recipientUserId) && recipientUserId.toString().length < 20) {
            ...
            // 2. Если это не число, проверяем в базе данных
            ...
            if (!isNaN(possibleId) && possibleId > 0) {
                ...
                // 3. Пробуем найти пользователя в базе данных
                // (В реальном приложении здесь должен быть поиск в БД)
                console.log('🔍 Ищем пользователя в базе данных по ID:', recipientUserId);
            }
        }

        if (!telegramId) {
            console.log('❌ Не удалось определить Telegram ID для пользователя:', recipientUserId);
            console.log('💡 Совет: Пользователь должен указать свой Telegram ID в профиле');
            return;
        }
        ...
        try {
            ...
            if (telegramError.response) {
                ...
                if (telegramError.response.error_code === 403) {
                    ...
                    console.log('💡 Пользователь должен запустить бота /start в Telegram');
        ...
        console.error('❌ Критическая ошибка в функции sendTelegramNotification:', error);
...
// Команда /start для бота (исправленная версия)
